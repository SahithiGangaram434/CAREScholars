...
descriptive_stats <- data_subset %>%
  ...
  summarise(
    ...
    Ownership_House_Percent = mean(OWNERSHIP == 10, na.rm = TRUE) * 100,
    Employment_Status_Percent = mean(EMPSTAT == 111, na.rm = TRUE) * 100,
    Insurance_Coverage_Percent = mean(HINOTCOVE == 1, na.rm = TRUE) * 100,
    Low_Income_Percent = mean(POORYN == 1, na.rm = TRUE) * 100,
    High_School_Education_Percent = mean(EDUCREC2 >= 2, na.rm = TRUE) * 100
  )

health_stats <- data_subset %>%
  group_by(racea, diabetes_status) %>%
  summarise(
    BMI_Mean = mean(BMI, na.rm = TRUE),
    BMI_SD = sd(BMI, na.rm = TRUE),
    Diabetes_Percent = mean(DIABETICEV == 1, na.rm = TRUE) * 100,
    Hypertension_Percent = mean(HYPERTENEV == 1, na.rm = TRUE) * 100,
    Stroke_Percent = mean(STROKEV == 1, na.rm = TRUE) * 100,
    Heart_Attack_Percent = mean(HEARTATTEV == 1, na.rm = TRUE) * 100,
    Physical_Activity_Percent = mean(MOD10DMIN > 0, na.rm = TRUE) * 100
  )

combined_stats <- left_join(descriptive_stats, health_stats, by = c("racea", "diabetes_status"))
