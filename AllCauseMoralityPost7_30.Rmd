...
data_practice<-data_practice%>%
  ...
  mutate(reacea2=relevel(racea2, "White"))%>%
  mutate(Smoking_recode=relevel(as_factor(Smoking_recode), "Never Smoked"))
cox_mod2<-coxph(Surv(exposure_birth, Diab_Und_ACM) ~ racea2 + sex + BMI_R_Cat+Education_recode+Earnings_recode+Smoking_recode+Insurance_recode, data = data_practice)
...
```{r}
data_racea<-with(data, data.frame())
